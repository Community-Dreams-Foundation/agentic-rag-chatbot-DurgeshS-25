...
### `app/retrieve.py` — Dense Retrieval
Finds the most semantically relevant chunks for a query.

- **Method:** FAISS inner product search on normalized vectors (cosine similarity)
- **Query embedding:** Same model as index — `embed_query()` returns normalized `(1, 384)` vector
- **Top-K:** Configurable, default 7 in UI
- **Validation:** Checks index/chunk alignment before search; clamps top-k to available vectors
- **Output:** List of chunk dicts sorted by descending similarity score, each with added `score` field
...
| Vector index | FAISS IndexFlatIP | Exact search, no approximation — correct for small-medium corpora |
